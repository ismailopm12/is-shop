-- ============================================
-- DYNAMIC USER INFO FIELDS SYSTEM
-- Run this ENTIRE script in Supabase SQL Editor
-- ============================================

-- PART 1: CREATE PRODUCT CATEGORIES TABLE (if not exists)
...
-- PART 2: ADD CATEGORY_ID TO PRODUCTS
...
-- PART 3: CHANGE user_info_fields from array to JSONB for flexible field management
-- First, remove old column if it exists as array
DO $$ 
BEGIN 
  IF EXISTS (
    ...
    WHERE table_schema = 'public' 
      ...
      AND column_name = 'user_info_fields'
      AND data_type = 'ARRAY'
  ) THEN
    ALTER TABLE public.products DROP COLUMN user_info_fields;
  END IF;
END $$;

-- Add new JSONB column for flexible field configuration
ALTER TABLE public.products 
ADD COLUMN IF NOT EXISTS user_info_fields JSONB DEFAULT '[]'::jsonb;
...
DROP FUNCTION IF EXISTS public.get_product_user_info_fields(TEXT);

-- Function to get product user info fields as JSONB
...
-- Drop existing save function if exists
DROP FUNCTION IF EXISTS public.save_user_info_with_order(UUID, JSONB);

-- Function to save user submitted info with order
CREATE OR REPLACE FUNCTION public.save_user_info_with_order(
  _order_id UUID,
  _user_info JSONB
)
RETURNS VOID AS $$
BEGIN
  ...
  SET user_info = _user_info
  WHERE id = _order_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- PART 7: ADD user_info column to orders table to store submitted data
...
-- Show products with their user info field configurations
SELECT 
